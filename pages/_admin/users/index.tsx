...
const AdminUsers: NextPage = ({ initialInquiry }: any) => {
  ...
  const textHandler = (value: string) => setSearchText(value);
  ...
  const searchTypeHandler = (newValue: string) => {
    ...
    if (newValue !== 'ALL') {
      ...
      const searchCopy = { ...membersInquiry.search };
      delete searchCopy.memberType;
      setMembersInquiry({ ...membersInquiry, search: searchCopy });
    }
  };
            ...
            <Box component={'div'}>
              ...
              <Stack className={'search-area'} sx={{ m: '24px' }}>
                ...
                <Select sx={{ width: '160px', ml: '20px' }} value={searchType}>
                  {['ALL', 'USER', 'AGENT', 'ADMIN'].map((type) => (
                    <MenuItem key={type} value={type} onClick={() => searchTypeHandler(type)}>
                      {type}
                    </MenuItem>
                  ))}
                </Select>
